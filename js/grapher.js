...
(function (Grapher) {
    var MotionChart = (function () {
        ...
        MotionChart.prototype.startTransition = function () {
            ...
            var self = this;
            this._diagram.transition().duration(duration).ease("linear").tween("date", function () {
                return function (t) {
        ...
        MotionChart.prototype.createScales = function () {
            ...
            var xScale = this._xScale.domain(xDomain).range([
                1.5 * maxRadius, 
                this._width - (1.5 * maxRadius) - maxLabelWidth
            ]);
            this._xScale = this._xScale.domain([
                xScale.invert(0), 
                xScale.invert(this._width)
            ]).range([
                0, 
                this._width
            ]).clamp(true);
            var yScale = this._yScale.domain(yDomain).range([
                1.5 * maxRadius, 
                this._height - (1.5 * maxRadius)
            ]);
            this._yScale = this._yScale.domain([
                yScale.invert(0), 
                yScale.invert(this._height)
            ]).range([
                this._height, 
                0
            ]).clamp(true);
            this._radiusScale = this._radiusScale.domain([
                0, 
                this._radiusDomain[1]
            ]).range([
                2, 
                maxRadius
            ]).clamp(true);
            if(this._colorDomain) {
                ...
                gradient.forEach(function (d) {
                    linearGradient.append("stop").attr("offset", d["stop"]).attr("stop-color", d["color"]);
                });
                var gradientStops = gradient.map(function (d) {
                    return d["stop"];
                });
                var gradientColors = gradient.map(function (d) {
                    return d["color"];
                });
        ...
        MotionChart.prototype.createRules = function () {
            ...
            rules.append("g").classed("grid", true).call(this.yAxis.tickSize(-this._width, 0, -this._width).tickFormat(function (value) {
                return "";
            }));
            rules.selectAll(".grid line").filter(function (d) {
                return d == 0;
            }).classed("origin", true);
        ...
        MotionChart.prototype.createColorAxis = function () {
            if(this._colorDomain) {
                ...
                var colorScale = d3.scale.linear().domain(this._colorDomain).range([
                    0, 
                    w
                ]);
                var colorTicks = [
                    ...
                    0.5, 
                    1
                ].map(d3.scale.linear().domain(this._colorDomain).invert);
        ...
        MotionChart.prototype.createTimeSlider = function () {
            ...
            var x = this._element.attr("width") - this._margin.right - w;
            var y = Number(this._element.attr("height")) - 30;
            this._timeScale = d3.time.scale().domain([
                this._startTime, 
                this._endTime
            ]).range([
                0, 
            ...
            ticks = ticks.map(d3.scale.linear().domain([
                this._startTime, 
                this._endTime
            ]).invert);
            var timeTicks = [];
            ...
            var timeAxis = d3.svg.axis().orient("bottom").scale(this._timeScale).tickSize(11, 0, 11).tickValues(timeTicks).tickFormat(function (value) {
            ...
            var rect = function () {
                return g.append("rect").attr("rx", 2).attr("ry", 2).attr("y", -1).attr("x", -3).attr("height", 6);
            };
            ...
            this._timeSliderPlayButton.on("click", function () {
        ...
        MotionChart.prototype.createItems = function () {
            var self = this;
            this._items = this._diagram.append("g").selectAll(".item").data(this._dataSource).enter().append("g").classed("item", true).each(function (item) {
        ...
        MotionChart.prototype.computeDomain = function (axis) {
            ...
            var min = d3.min(this._dataSource, function (item) {
                return (typeof item[axis] == "number") ? item[axis] : d3.min(item[axis], function (pair) {
                    return pair[1];
                });
            });
            var max = d3.max(this._dataSource, function (item) {
                return (typeof item[axis] == "number") ? item[axis] : d3.max(item[axis], function (pair) {
                    return pair[1];
                });
            });
            self._dataSource.forEach(function (item) {
                if(item[axis] instanceof Array) {
                    var dates = item[axis].map(function (d) {
                        ...
                        return self.createDate(date);
                    });
                    var values = item[axis].map(function (d) {
                        return d[1];
                    });
        ...
        MotionChart.prototype.update = function (date) {
            ...
            this._items.each(function (data) {
                if(self.hasValue(data, self._xData, date) && self.hasValue(data, self._yData, date) && self.hasValue(data, self._radiusData, date)) {
