  ...
  function color_path(d, ctx) {
    ...
    __.dimensions.map(function(p,i) {
      if (i == 0) {
        ...
        ctx.lineTo(position(p),yscale[p](d[p])); //straight line
      }
    });
    ctx.stroke();
  };
...
d3.parcoords.detectDimensionTypes = function(data) {
  ...
  d3.keys(data[0])
    .forEach(function(col) {
      if (col == "STATION_COST" || col == "DMA_COST" || col == "STATION_VALUE" || col == "DMA_VALUE" || col == "result_payment" || col == "bid_amt_off" || col == "bid_amt_lvhf" || col == "bid_amt_uvhf"){
        types[col] = "log";
      } else{
